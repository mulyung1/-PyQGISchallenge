'''in this script, we write attributes of 
a vector layer to a csv file using the pyqgis api'''

import os
import time

data_dir = os.path.join(os.path.expanduser('~'), 'Downloads', 'pyqgis-masterclass','data')

layer = iface.activeLayer()

# Check if a layer is selected
if not layer:
    iface.messageBar().pushMessage('Not again!!, select a layer please',  level=Qgis.Critical) 
# Check if the selected layer is a vector layer
if layer.type() != QgsMapLayer.VectorLayer:
    iface.messageBar().pushMessage('Bruuh!!, Please select a vector layer',  level=Qgis.Critical)

# Define parameters for QgsVectorFileWriter
output_name = 'output3.csv'
